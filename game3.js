...
function checkAnswer(currentLevel) {
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    if (userClickedPattern.length === gamePattern.length) {
      ...
      levelLogs.push(`${currentTime};${level};${timeTaken};1;3;${playerNumber}`);
    ...
    const timeTaken = ((Date.now() - levelStartTime) / 1000);
    const currentTime = getCurrentTimestamp();
    levelLogs.push(`${currentTime};${level};${timeTaken};0;3;${playerNumber}`);
...
function saveToFile() {
  ...
  const fileName = `Gracz_${playerNumber}_Gra3_logs.csv`; 
